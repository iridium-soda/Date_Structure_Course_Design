...
int* DPLL(cnf* input)//If success,return int*->solution;if fail or error,return NULL
{
	...
	int endflag = 0;
	int track_flag = 0;
	Node* neg_node=(Node*)malloc(sizeof(Node));
	...
	do
	{
		...
		if (track_flag == 0)
		{
			if (unit = scan_single(matrix, input->clauses, input->units), unit != 0)//Have single clause
			{
				...
				solutioncheck(current_solution, input->units);
...
void solutioncheck(int* solution, int units)//Check and correct the solution
{
	int i;
	for (i = 0; i < units; i++)
	{
