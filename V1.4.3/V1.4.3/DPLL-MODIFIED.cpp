#include "head.h"
int* DPLL(cnf* input,  char* filename)
{
	int i, j, k;
	int status=0;
	int endflag = 0;
	...
	do
	{
		...
		if (endflag == 1)
		{
			...
			if (check(new_matrix, input->solutions, orignrows, input->units) == 1)
			{
				...
				status = filewirite(filename, input, ((end * 1000 - start * 1000) / CLOCKS_PER_SEC), 1);
				if (status == 1)
					printf("[SUCCESS] Log file written\n");
				else
					printf("[ERROR] Log file written failed\n");
		...
		else if (endflag == -1)
		{
			...
			if (top == &stack[0])
			{
				...
				printf("[TIME] The process takes %lf mSec\n", ((end * 1000 - start * 1000) / CLOCKS_PER_SEC));
