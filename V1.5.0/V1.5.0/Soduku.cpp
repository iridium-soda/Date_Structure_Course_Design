...
{//Random staeagty:add payload to secure rendomly enough and cover time-seed's influence
	...
	/*Generation*/
	int** sudoku_square = (int**)malloc(sizeof(int*) * 9);

	for (i = 0; i < 9; i++)
		sudoku_square[i] = (int*)calloc(9, sizeof(int));
kksk:
	sudo = sudokugenerating(sudoku_square);
	if (sudo == NULL)
	{
		cleansquare(sudoku_square);
		goto kksk;
	}

	system("PAUSE");
	system("cls");
	printf("[INFO] Stage V: sudoku\n");//Start sign
	//fileSimulator(sudo);

	printf("[INFO] Calculating...\n");
	int* solution = DPLL_revise(sudo, timecost);
	if (solution == NULL)
	{
	...
	else
	{
		...
		showsudoku(finalsquare);
	}
	//Problem occured: digit arrangement error. To prevent ,have to move the digit
	getchar();
}

cnf* sudokugenerating(int** square)//To generate sudoku randomly
{
	int i, j;
	long payload = 0;
	int timemark = 0;
...
void showsudoku(int** square)//show final square for test
{
	int i, j;
	for (i = 0; i < ROW; i++)
	{
